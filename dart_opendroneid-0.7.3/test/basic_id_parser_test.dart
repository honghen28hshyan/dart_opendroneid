...
void compareWithReference(
  ...
  using((arena) {
    final dartParser = BasicIDMessageParser(messageData);
...
void main() {
  ...
  group('Invalid data producing warnings', () {
    test('invalid id type', () {
      ...
      final parser = BasicIDMessageParser(messageData);

      parser.parse();

      expect(parser.hasWarnings, isTrue);
      expect(parser.warnings.first.message, WarnMessages.unrecognizableUASID);
    });
  });
}
