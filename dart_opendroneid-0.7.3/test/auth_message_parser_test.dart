...
void compareWithReference(
  ...
  using((arena) {
    final dartParser = AuthMessageParser(messageData);
    final nativeParser = NativeParser(arena);

    final dartOutput = dartParser.parse();
    final nativeOutput = nativeParser.parseAuth(messageData);

    expect(dartParser.hasWarnings, isFalse);
...
void main() {
  ...
  group('Invalid data producing warnings', () {
    test('invalid length - zero pages', () {
      final Uint8List messageData = AuthMessages.invalidAuthLenZeroPage;
      final parser = AuthMessageParser(messageData);

      parser.parse();

      expect(parser.hasWarnings, isTrue);
      expect(parser.warnings.first.message,
          WarnMessages.authenticationMessageExceedsMaxSizes);
    });

    test('invalid auth type', () {
      final Uint8List messageData = AuthMessages.invalidAuthType;
      final parser = AuthMessageParser(messageData);
      ...
      expect(parser.hasWarnings, isTrue);
      expect(parser.warnings.first.message,
          WarnMessages.unknownFieldValue('AuthType', actualValue: 9));
    });
  });
}
